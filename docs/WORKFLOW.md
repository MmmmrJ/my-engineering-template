...
- 新增或改变页面、用户流程、交互、响应式布局或视觉设计时，UI 角色必须在方案阶段提交 `docs/design/<feature>/design.md` 与 `prototypes/` 下的本地原型图；前端实现前运行 `validate-design`，最终 QA 将实现截图和偏差记录写入 `verification.md` 后运行 `validate-visual`

...
description: Implement APIs, domain rules, data models, migrations, provider adapters, and backend tests with explicit contracts and failure modes. Use when changing server behavior, persistence, data processing, integrations, or backend tests. Do not use for client-only presentation changes.
---
...
4. 明确精度、幂等性、并发、事务、审计、状态机和失败恢复规则。
5. 隔离外部供应商或第三方适配层；不得把演示/缓存数据伪装为权威实时数据。

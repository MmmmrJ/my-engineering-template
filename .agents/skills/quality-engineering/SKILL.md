...
2. 阅读已确认的产品验收、UI 状态、API 契约、实现改动和已有测试。涉及 UI 时，设计目录中的 `design.md` 与本地原型图也是验收基线；先运行 `validate-design`，再核对关键页面、状态、交互、响应式和无障碍实现。
...
8. 涉及 UI 时，写入 `verification.md`：逐项映射原型图、固定视口、测试数据和实现截图；记录偏差的原因、影响、UI 确认人和设计版本，并运行 `validate-visual`。给出通过、阻塞或不通过结论，并列出确认版本、未覆盖项和剩余风险。

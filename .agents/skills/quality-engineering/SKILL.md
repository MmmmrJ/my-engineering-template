...
1. 区分方案阶段和最终验收阶段：方案阶段只能只读检查并输出测试计划；最终验收还必须读取唯一 active task 的 `governance.json`，确认版本一致、实现角色已完成、阶段允许验收且测试路径属于自己的允许范围。
...
- 不得推进任务阶段、修改父 Agent 的治理结论、自行接受风险或宣布任务完成；只返回 QA 结论及可写入 `acceptance.results` 的检查证据。
